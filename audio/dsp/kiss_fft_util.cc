...
void FftHardBandpass(const std::vector<float>& input, float low_band_edge,
  ...
  CHECK_LT(high_band_edge, sample_rate * (0.5f + 0.5f / input.size()));
...
void HardBandpassSpectrum(const std::vector<complex<float>>& input_spectrum,
                          ...
                          float bin_width,
                          std::vector<complex<float>>* output_spectrum) {
  CHECK(!input_spectrum.empty());
  const int spectrum_size = static_cast<int>(input_spectrum.size());
  CHECK(output_spectrum != nullptr);
  CHECK_GE(low_band_edge, 0.0f);
  CHECK_LE(low_band_edge, high_band_edge);
  CHECK_LE(high_band_edge, bin_width * spectrum_size);
...
void FftPeriodicResample(const std::vector<float>& input,
                         ...
                         std::vector<float>* output) {
  CHECK_GT(input_sample_rate, 0);
  CHECK_GT(output_sample_rate, 0);
  CHECK(output);
