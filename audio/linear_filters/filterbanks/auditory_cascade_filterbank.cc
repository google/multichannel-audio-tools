...
void CheckParameterValues(const AuditoryCascadeFilterbankParams& params,
  ...
  CHECK_GT(params.highest_pole_frequency(), 0.0);
  CHECK_GE(params.max_pole_nyquist_fraction(), 0.0);
  CHECK_LE(params.max_pole_nyquist_fraction(), 1.0);
  CHECK_GT(params.step_erbs(), 0.0);
  CHECK_LT(params.step_erbs(), 2.0);
  CHECK_GT(params.pole_zeta(), 0.0);
  CHECK_LT(params.pole_zeta(), 0.5);
  CHECK_GT(params.min_pole_frequency(), 0.0);
  if (!params.channel_selection_options().use_all_channels()) {
    CHECK_GE(params.channel_selection_options().first_channel(), 0);
    CHECK_GT(params.channel_selection_options().skip_every_n(), 0);
    CHECK_GT(params.channel_selection_options().max_channels(), 0);
  }
  if (is_decimation_allowed) {
    CHECK_GT(params.max_samples_per_cycle(), 6.0);
    CHECK_GT(params.min_sample_rate(), 0.0);
  }
}
...
void AuditoryCascadeFilterbank::DesignFilterbank(float full_sample_rate_hz) {
  CHECK_GT(full_sample_rate_hz, 0.0);
